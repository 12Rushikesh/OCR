...
# -----------------------------
QWEN_OV_PATH = r"E:\ocr\flocr\qwen3-vl-2b-fp16-ov"  # path to exported OpenVINO IR directory
...
try:
    ...
    raise RuntimeError(f"Failed to load OpenVINO model at {QWEN_OV_PATH}: {e}") from e
...
def qwen_ocr_raw(pil_image: Image.Image) -> str:
    ...
    try:
        ...
        if hasattr(out_ids, "tolist"):
            ...
            generated_ids = out_ids[0][prompt_len:].tolist() if out_ids.ndim == 2 else out_ids[prompt_len:].tolist()
            # Let processor decode the generated ids back to string (if tokenizer present)
            # But simplest: use processor.decode if available, else fall back to processor.tokenizer
            decoded = processor.decode(generated_ids, skip_special_tokens=True) if hasattr(processor, "decode") else processor.tokenizer.decode(generated_ids, skip_special_tokens=True)
        else:
            decoded = str(out_ids)
    except Exception:
